...
output:
  html_document:
    ...
    toc_depth: 4
    fig_width: 10
date: 'Last updated on `r format(Sys.time(), "%B %d, %Y")` to include data from the previous day'
---
...
stay_dates[, end_date := as.Date(end_date, format = "%Y-%m-%d")]
...
cases.agg[, daysDoubleBin := ifelse(daysToDouble < 3, "Less than 3 days",
                                    ifelse(daysToDouble >= 3 & daysToDouble < 7, "3-7 days",
                                           ifelse(daysToDouble >= 7 & daysToDouble < 14, "1-2 weeks",
                                                  ifelse(daysToDouble >= 14 & daysToDouble < 28, "2-4 weeks", "More than 4 weeks"))))]

# try a plot!
use.cases <- cases.agg[date >= "2020-03-01" & !is.na(daysDoubleBin) & cases >= 50]
use.cases[, daysDoubleBin := factor(daysDoubleBin, levels = c("Less than 3 days", "3-7 days", "1-2 weeks", "2-4 weeks", "More than 4 weeks"))]
...
deaths.agg[, daysDoubleBin := ifelse(daysToDouble < 3, "Less than 3 days",
                                    ifelse(daysToDouble >= 3 & daysToDouble < 7, "3-7 days",
                                           ifelse(daysToDouble >= 7 & daysToDouble < 14, "1-2 weeks",
                                                  ifelse(daysToDouble >= 14 & daysToDouble < 28, "2-4 weeks", "More than 4 weeks"))))]
...
use.deaths[, daysDoubleBin := factor(daysDoubleBin, levels = c("Less than 3 days", "3-7 days", "1-2 weeks", "2-4 weeks", "More than 4 weeks"))]
...
# make single day total growth rates
temp <- agg.cases.US
...
# make single day new growth for rolling 3-day average
temp <- agg.cases.US
agg.cases.US[, total_cases_prev7 := temp$total_cases[match(agg.cases.US$date - 7, temp$date)]]
agg.cases.US[, avg_new_cases_prev7 := (total_cases - total_cases_prev7)/7]
temp <- agg.cases.US
agg.cases.US[, avg_new_cases_prev7_prev := temp$avg_new_cases_prev7[match(agg.cases.US$date - 1, temp$date)]]
agg.cases.US[, new_cases_rtgrowth := avg_new_cases_prev7/avg_new_cases_prev7_prev - 1]
...
agg.cases.growth.US.plot <- ggplot(melt.use[date >= "2020-03-16"], aes(x = date, y = value, color = variable)) + 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    labs(x = "Date",
       y = "Growth rate (see comments above for more detail)",
       title = "Growth rates: Confirmed COVID cases",
...
```{r echo = F, message = F, warning = F}
use.states <- c("Washington", "California", "South Carolina", "Tennessee")

for (i in 1:length(use.states)) {
  use.state <- use.states[i]
  cases.state <- state.raw[state %in% use.state]
  ...
  agg.cases.state[, avg_new_cases_prev7 := (total_cases - total_cases_prev7)/7]
  temp <- agg.cases.state
  agg.cases.state[, avg_new_cases_prev7_prev := temp$avg_new_cases_prev7[match(agg.cases.state$date - 1, temp$date)]]
  agg.cases.state[, new_cases_rtgrowth := avg_new_cases_prev7/avg_new_cases_prev7_prev - 1]
  ...
  agg.cases.state[, daysToDouble := log(2, 1 + total_cases_rtgrowth_rolling3)]
  ...
  agg.cases.growth.state.plot <- ggplot(melt.use[date >= "2020-03-16"], aes(x = date, y = value, color = variable)) + 
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = 
                         c(ifelse(min(melt.use$value) > 0, 0, min(melt.use$value)), max(melt.use$value * 1.1)))
  print(agg.cases.growth.state.plot)
}
```
...
agg.deaths.US[, avg_new_deaths_prev7 := (total_deaths - total_deaths_prev7)/7]
temp <- agg.deaths.US
agg.deaths.US[, avg_new_deaths_prev7_prev := temp$avg_new_deaths_prev7[match(agg.deaths.US$date - 1, temp$date)]]
agg.deaths.US[, new_deaths_rtgrowth := avg_new_deaths_prev7/avg_new_deaths_prev7_prev - 1]
...
agg.deaths.growth.US.plot <- ggplot(melt.use[date >= "2020-03-16"], aes(x = date, y = value, color = variable)) + 
  ...
  geom_hline(yintercept = 0, linetype = "dashed", color = "black") +
    labs(x = "Date",
       y = "Growth rate (see comments above for more detail)",
       title = "Growth rates: COVID deaths",
       subtitle = "United States",
       color = "Metric") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
...
for (i in 1:length(use.states)) {
  ...
  agg.deaths.state[, avg_new_deaths_prev7 := (total_deaths - total_deaths_prev7)/7]
  temp <- agg.deaths.state
  agg.deaths.state[, avg_new_deaths_prev7_prev := temp$avg_new_deaths_prev7[match(agg.deaths.state$date - 1, temp$date)]]
  agg.deaths.state[, new_deaths_rtgrowth := avg_new_deaths_prev7/avg_new_deaths_prev7_prev - 1]
  ...
  agg.deaths.state[, daysToDouble := log(2, 1 + total_deaths_rtgrowth_rolling3)]
  ...
  agg.deaths.growth.state.plot <- ggplot(melt.use[date >= "2020-03-16"], aes(x = date, y = value, color = variable)) + 
    ...
    scale_y_continuous(labels = scales::percent_format(accuracy = 1), limits = 
                         c(ifelse(min(melt.use$value) > 0, 0, min(melt.use$value)), max(melt.use$value * 1.1)))
  print(agg.deaths.growth.state.plot)
}
```
...
state.per.cap.agg <- state.per.cap[date == date.cut & !is.na(casesPerMillion)]
state.per.cap.agg <- state.per.cap.agg[order(-casesPerMillion), c("state", "casesPerMillion")]
state.per.cap.agg[, casesPerMillion := comma(casesPerMillion, accuracy = 1)]
setnames(state.per.cap.agg, c("state", "casesPerMillion"), c("State", "Cases Per Million"))
state.per.cap.agg[, Ranking := .I]
state.per.cap.agg <- state.per.cap.agg[, c(3, 1, 2)]
kable(state.per.cap.agg, align = "l") %>%
...
state.per.cap.agg <- state.per.cap.agg[!is.na(casesNewPerMillion)]
state.per.cap.agg[, casesNewPerMillion := comma(casesNewPerMillion, accuracy = 1)]
setnames(state.per.cap.agg, c("state", "casesNewPerMillion"), c("State", "New Cases Per Million"))
state.per.cap.agg[, Ranking := .I]
state.per.cap.agg <- state.per.cap.agg[, c(3, 1, 2)]
kable(state.per.cap.agg, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), fixed_thead = T, full_width = F, position = "left") %>%
...
date.cut <- max(state.per.cap$date)
state.per.cap.agg <- state.per.cap[date == date.cut & !is.na(deathsPerMillion)]
state.per.cap.agg <- state.per.cap.agg[order(-deathsPerMillion), c("state", "deathsPerMillion")]
state.per.cap.agg[, deathsPerMillion := comma(deathsPerMillion, accuracy = 1)]
setnames(state.per.cap.agg, c("state", "deathsPerMillion"), c("State", "Deaths Per Million"))
state.per.cap.agg[, Ranking := .I]
state.per.cap.agg <- state.per.cap.agg[, c(3, 1, 2)]
kable(state.per.cap.agg, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), fixed_thead = T, full_width = F, position = "left") %>%
...
state.per.cap.agg <- state.per.cap.agg[!is.na(deathsNewPerMillion)]
state.per.cap.agg <- state.per.cap.agg[order(-deathsNewPerMillion), c("state", "deathsNewPerMillion")]
state.per.cap.agg[, deathsNewPerMillion := comma(deathsNewPerMillion, accuracy = 1)]
setnames(state.per.cap.agg, c("state", "deathsNewPerMillion"), c("State", "New Deaths Per Million"))
state.per.cap.agg[, Ranking := .I]
state.per.cap.agg <- state.per.cap.agg[, c(3, 1, 2)]
kable(state.per.cap.agg, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), fixed_thead = T, full_width = F, position = "left") %>%
...
table.data <- county.per.cap[flgOurCounty == 1 | rawRanking < 6, -c("flgOurCounty")]
setnames(table.data, c("county", "state", "casesPerMillion", "rawRanking", "percentile"),
         c("County", "State", "Cases Per Million", "Raw Ranking", "Percentile"))
kable(table.data, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, fixed_thead = T, position = "left") %>%
  column_spec(1)
```
...
county.per.cap[, flgOurCounty := ifelse(county %in% "Pierce" & state %in% "Washington", 1,
                                       ifelse(county %in% "Orange" & state %in% "California", 1,
                                              ifelse(county %in% c("Richland", "York") & state %in% "South Carolina", 1,
                                                     ifelse(county %in% "Davidson" & state %in% "Tennessee", 1, 0))))]
table.data <- county.per.cap[flgOurCounty == 1 | rawRanking < 6, -c("flgOurCounty")]
setnames(table.data, c("county", "state", "deathsPerMillion", "rawRanking", "percentile"),
         c("County", "State", "Deaths Per Million", "Raw Ranking", "Percentile"))
kable(table.data, align = "l") %>%
  kable_styling(bootstrap_options = c("striped", "hover", "condensed", "responsive"), full_width = F, fixed_thead = T, position = "left") %>%
  column_spec(1)
```
...
use.states <- c("Washington", "California", "South Carolina", "South Carolina", "Tennessee")
use.counties <- c("Pierce", "Orange", "Richland", "York", "Davidson")
for (i in 1:length(use.counties)) {
  use.state <- use.states[i]
  use.county <- use.counties[i]
...
for (i in 1:length(use.states)) {
  ...
  agg.new.cases.state.plot <- ggplot(agg.cases.state[date >= "2020-03-01"]) + 
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          legend.position = "top") +
    scale_y_continuous(labels = comma)
  print(agg.new.cases.state.plot)
}
```
...
for (i in 1:length(use.counties)) {
  ...
  agg.new.cases.county.plot <- ggplot(agg.cases.county[date >= "2020-03-01"], aes(x = date, y = total_new_cases)) + 
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          legend.key = element_blank()) +
    scale_y_continuous(labels = comma)
  print(agg.new.cases.county.plot)
}
```
...
for (i in 1:length(use.states)) {
  ...
  agg.new.deaths.state.plot <- ggplot(agg.deaths.state[date >= "2020-03-01"], aes(x = date, y = total_new_deaths)) + 
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          legend.key = element_blank(),
          legend.position = "top")
  print(agg.new.deaths.state.plot)
}
```
...
for (i in 1:length(use.counties)) {
  ...
  agg.new.deaths.county.plot <- ggplot(agg.deaths.county[date >= "2020-03-01"], aes(x = date, y = total_new_deaths)) + 
    ...
    theme(plot.title = element_text(hjust = 0.5),
          ...
          axis.line = element_line(colour = "black"),
          legend.key = element_blank())
  print(agg.new.deaths.county.plot)
}
```
...
state.dt[, end_date := stay_dates$end_date[match(state.raw$state, stay_dates$state)]]
state.dt[, end_date := ifelse(is.na(end_date), as.Date("2099-01-01"), end_date)]
state.dt[, flgOrder := ifelse(is.na(stay_date) | stay_date > date | end_date <= date, "No", "Yes")]
temp <- state.dt
temp <- temp[, c("date", "state", "cases")]
setnames(temp, "cases", "cases_prev3")
state.dt[, dtPrev3 := date - 3]
state.dt <- merge(state.dt, temp, all.x = T, by.x = c("state", "dtPrev3"), by.y = c("state", "date"))
state.dt[, casesNewPerMillion := as.integer((cases - cases_prev3)/population * 1000000)]

# temp <- state.dt
# temp <- temp[, c("date", "state", "cases")]
# setnames(temp, "cases", "cases_prev")
# state.dt[, dtPrev := date - 1]
# state.dt <- merge(state.dt, temp, all.x = T, by.x = c("state", "dtPrev"), by.y = c("state", "date"))
# state.dt[, new_cases_rtgrowth := cases/cases_prev - 1]
# 
# temp <- state.dt[, c("date", "state", "new_cases_rtgrowth")]
# setnames(temp, "new_cases_rtgrowth", "new_cases_rtgrowth_prev1")
# state.dt <- merge(state.dt, temp, all.x = T, by.x = c("state", "dtPrev"), by.y = c("state", "date"))
# 
# temp <- state.dt[, c("date", "state", "new_cases_rtgrowth")]
# setnames(temp, "new_cases_rtgrowth", "new_cases_rtgrowth_prev2")
# state.dt[, dtPrev2 := date - 2]
# state.dt <- merge(state.dt, temp, all.x = T, by.x = c("state", "dtPrev2"), by.y = c("state", "date"))
# 
# state.dt[, new_cases_rtgrowth_rolling3 := (new_cases_rtgrowth + new_cases_rtgrowth_prev1 + new_cases_rtgrowth_prev2)/3]
...
state.agg.comp.plot <- ggplot(state.agg.comp, aes(x = date, y = avgNewCases, colour = as.factor(flgOrder))) +
  ...
  geom_line(aes(group = flgOrder)) +
    labs(x = "Date",
       y = "Average of new cases per million resident",
       title = "New COVID case rates compared to stay-at-home order",
...
state.stay.comp <- state.stay.comp[!is.na(new_cases_rtgrowth_rolling3),
                                   .(avgNewCaseGrowth = mean(new_cases_rtgrowth_rolling3, na.rm = T),
                                     cts = .N),
                                   by = c("daysFromStayOrder")]

state.stay.comp.plot <- ggplot(state.stay.comp, aes(x = daysFromStayOrder, y = avgNewCaseGrowth)) +
  geom_line() +
  geom_point(aes(size = cts)) +
    labs(x = "Days from stay-at-home order",
       y = "Average new case growth rate",
       title = "New COVID cases by stay-at-home order timing",
       subtitle = "New case growth rate is a rolling 3-day average",
       size = "Count of states") +
  theme(plot.title = element_text(hjust = 0.5),
        plot.subtitle = element_text(hjust = 0.5),
        panel.grid.major = element_blank(),
