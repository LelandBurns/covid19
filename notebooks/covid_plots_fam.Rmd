...
* The page starts by showing growth rates. I've moved down the raw numbers to later in the page because you can see raw numbers everywhere, and it's sometimes hard to understand trends by looking only at them. Growth rates seem to me to be more important for understanding our progress.
  * For the U.S. and each of our states, there are two growth rate charts: confirmed cases and deaths. Each chart then has two lines: one for total cases or deaths, and one for new cases or deaths. Each of these lines is a rolling three-day average of daily growth rates for that particular metric. Note that I recently changed this so that each line is an avergae of *daily* growth rates, which better aligns the metrics, but can also make some of the trendlines a bit bumpier.
    * For total cases (or deaths), the trendlines will be a bit smoother and more consistent. We want to see these decline (and almost all are), but they can't go below zero because total cases (or deaths) can never go down. We at least want them to get as close to zero as possible.
    * For new cases (or deaths), the lines are a bit bumpier, in particular for states. While the trends aren't as smooth, we want to wtach for them to get consistently below zero and stay there. That means that we are consistently seeing less new cases (or deaths) on a daily basis and that we're on the other side of the "apex."
...
* The section is a work in progress -- I'll continue to update over the coming days.
...
state.agg.comp.plot <- ggplot(state.agg.comp, aes(x = date, y = avgNewCases, colour = as.factor(flgOrder))) +
  ...
  geom_line(aes(group = flgOrder)) +
    labs(x = "Date",
       ...
       subtitle = "Rolling 3-day average of new cases per million residents",
